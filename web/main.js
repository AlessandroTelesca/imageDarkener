...
const outputSlidersRoot = mustElement("outputSliders");
const minimumSlidersRoot = mustElement("minimumSliders");
const outputLock = mustElement("outputLock");
const minimumLock = mustElement("minimumLock");
const imageUpload = mustElement("imageUpload");
const previewCanvas = mustElement("previewCanvas");
const referenceImage = mustElement("referenceImage");
const saveBtn = mustElement("saveBtn");
const resetBtn = mustElement("resetBtn");
const pickColorBtn = mustElement("pickColorBtn");
const statusText = mustElement("statusText");
...
function mustCanvasContext(canvas) {
...
function buildSliderGroup(group, root) {
...
function onSliderChange(group, source, value) {
...
function syncTriplet(group, source, value) {
    isSyncing = true;
    const channels = ["r", "g", "b"];
    for (const channel of channels) {
        if (channel === source) {
...
function setPickColorMode(enabled) {
...
function sampleFromClientPosition(clientX, clientY) {
...
function releaseReferenceImageUrl() {
    if (!referenceImageUrl) {
        return;
    }
    URL.revokeObjectURL(referenceImageUrl);
...
function getSliderValue(group, channel) {
...
function onReset() {
    Object.keys(sliderMap).forEach((key) => {
        const [group, channel] = key.split("_");
